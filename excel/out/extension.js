...
function activate(context) {
    ...
    const createExcel = vscode.commands.registerCommand('mcp.createExcel', async () => {
        ...
        const data = await vscode.window.showQuickPick([{ label: "", description: "No initial data" }, ...familyOf4], { placeHolder: "Select data row (optional)" });
        if (!filename || !columnSet) {
            vscode.window.showErrorMessage("Filename and Columns are required.");
            return;
        }
        try {
            const response = await axios_1.default.post("http://localhost:5000/createExcel", {
                filename: filename.label,
                columns: columnSet.label.split(",").map((c) => c.trim()),
                data: data?.label ? data.label.split(",").map((d) => d.trim()) : []
            });
    ...
    const addFamilyOf4 = vscode.commands.registerCommand('mcp.addFamilyOf4', async () => {
        ...
        try {
            const response = await axios_1.default.post("http://localhost:5000/addFamilyOf4", {
                filename: filename.label,
                region: region.label, // ✅ must be here
                row: familyRow.label.split(",").map((d) => d.trim())
            });
    ...
    const addFamilyOf2 = vscode.commands.registerCommand('mcp.addFamilyOf2', async () => {
        ...
        try {
            const response = await axios_1.default.post("http://localhost:5000/addFamilyOf2", {
                filename: filename.label,
                row: familyRow.label.split(",").map((d) => d.trim())
            });
    ...
    const addSubscriber = vscode.commands.registerCommand('mcp.addSubscriber', async () => {
        ...
        try {
            const response = await axios_1.default.post("http://localhost:5000/addSubscriber", {
                filename: filename.label,
                row: subscriberRow.label.split(",").map((d) => d.trim())
            });
    ...
    const createTicket = vscode.commands.registerCommand('mcp.createTicket', async () => {
        ...
        try {
            const response = await axios_1.default.post("http://localhost:5000/ticket", {
                title: title.label,
    ...
    context.subscriptions.push(createExcel, addFamilyOf4, addFamilyOf2, addSubscriber, createTicket);
}
